...
# /pm:build - Project Building and Packaging
...
## Usage
```
/pm:build [target] [--type dev|prod|test] [--clean] [--optimize] [--verbose]
```
...
## Agent Coordination
- **devops-engineer**: Activated for build optimization and deployment preparation
- **performance-engineer**: Build artifact optimization and bundle size analysis
- If Playwright MCP available: Build validation and UI testing during builds
...
```
/pm:build --type prod --clean --optimize
...
```
/pm:build --type dev --validate
# Development build with testing validation if Playwright available
