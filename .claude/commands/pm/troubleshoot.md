...
# /pm:troubleshoot - Issue Diagnosis and Resolution
...
## Usage
```
/pm:troubleshoot [issue] [--type bug|build|performance|deployment] [--trace] [--fix]
```
...
```
/pm:troubleshoot "Null pointer exception in user service" --type bug --trace
...
```
/pm:troubleshoot "TypeScript compilation errors" --type build --fix
...
```
/pm:troubleshoot "API response times degraded" --type performance
...
```
/pm:troubleshoot "Service not starting in production" --type deployment --trace
